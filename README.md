...
| Claim-safe owner saves | A person who joins a share link while the owner still has the page open can no longer be silently deleted by the owner's next save. If a name-not-in-payload joiner is still present, the save is refused with a reload prompt instead of erasing them |
