...
| Guest profile cleanup | Guests no longer see a Delete account option on `/profile` that never applied to them — real accounts can still delete. The dashboard header and the guest "create an account" prompt both got breathing room added above them |
...
| First-deploy checklist scope (OT-159) | Fix `vercel.json`'s invalid JSON and stop there, or also try to flip `deploymentEnabled` / link the Vercel project / set env values | Fixed `vercel.json` only (the JSON syntax error had already been committed to main separately in `820594f`) and documented the rest as an owner checklist in `docs/deployment.md` — an agent cannot read or set Vercel/Supabase dashboard values, so pretending to close that half would have marked the task done on the owner's behalf |
| Fleet guard proof harness scope (OT-160) | Rebuild the case harness and also fix the guard hook's remaining known gaps in the same pass | Rebuilt only the proof — `scripts/protect-fleet-cases.sh`, 68 cases, hook untouched. Known-open edges (filed as OT-154) are asserted at the hook's *current* behavior and tagged `[OT-154#n]`, so fixing OT-154 later flips those cases deliberately instead of silently passing a weaker bar |
| Fixture strategy for the guard proof (OT-160) | Reuse this repo's own worktrees for the grant / no-grant cases | Build a throwaway git repo, its own worktrees, and its own maintenance grant under `mktemp -d`, torn down on exit including on failure — the previous version of this harness depended on this repo's real worktrees (`wt-OT-147`, `wt-OT-150`) and rotted the moment those were cleaned up |
| Guard-harness hook timeout (OT-161) | A hand-rolled bash timeout (background the hook, sleep, kill) | `perl -e 'alarm shift; exec @ARGV'`, exiting 142 on a hang, counted as a failed case — the hand-rolled version cost a whole builder attempt before landing on this |
| Loop-hook fallback trigger (OT-156) | Keep gating the transcript-scan fallback on `last_assistant_message` being falsy (`// ""`) | Gate on the field being absent (`has()`), not empty or null — an empty reply is real, not a signal to fall back, and treating it as one let a stale marker still in the transcript force a false continuation |
...
- **A proof that lives only in `/tmp` is not a proof, it's a memory of one.** OT-150 and OT-152's case harnesses verified the patched fleet guard 53/53 then 96/96, but both lived under `/tmp` and were gone by the time anyone needed to re-run them — all that was left was a claim in the ledger, not something executable. Versioning the harness in the repo is what makes "68 ok / 0 fail" a fact anyone can reproduce, rather than an assertion to trust.
...
- **A silent redirect fallback looks identical to a working sign-in.** Supabase does not error on an unlisted `emailRedirectTo` — it falls back to the project's Site URL, so the form submits, the magic link arrives, and the user lands somewhere with no session and nothing in the logs. Nothing in the app's own code paths could have caught this; it only surfaces by knowing the dashboard's allowlist behavior and checking for it explicitly. Any auth-adjacent env or config surface deserves the same explicit "what does silent failure look like" pass, not just "does the code path work."

- **macOS has no `timeout` command.** A hand-rolled bash timeout for the guard-harness hook check cost a whole builder attempt before the fix landed on `perl -e 'alarm shift; exec @ARGV'` — portable, one line, exits 142 on a hang.

- **Absent, null, and empty are three different signals, not one.** `jq -r '.field // ""'` collapses all three to the same string, which is fine until one of them means something different from the others — here, "field truly missing" should trigger a fallback and "field present but empty" should not. Collapsing them let a stale marker in the transcript silently strand unfinished work.
