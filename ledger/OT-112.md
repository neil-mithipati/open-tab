...
files:
  ...
  - .gitignore
blocked_reason: >-
  attempt 2 dispatch was DENIED by the parallel cap — "9 builder(s) already
  running - MAX_PARALLEL=8". this is the OT-114 counter drift, and this time it
  is genuinely binding, not a stale projection. only 2 builders are actually
  running; 5 of the 9 are stale records. blocked on OT-114 merging, which makes
  the count self-healing. the partial work in ../wt-OT-112 is intact and safe to
  build on. attempts stays 1: no attempt 2 ever ran.
---
