...
blocked_reason: >-
  work is COMPLETE and committed (af67ef3): both fixes landed, lint clean, tests
  187/187. blocked only on the required `typecheck` gate, which is red on main
  from two pre-existing errors in test files outside this task's scope. OT-116
  fixes exactly those. sequence: merge OT-116, merge main into this branch,
  re-run gates, review, merge. do not mark done until typecheck is green.
---
