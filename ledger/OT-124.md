...
blocked_reason: >-
  depends on OT-123 merging first. both edit src/lib/rateLimit.ts — OT-123 extends
  the parse limiter's gate, this task must repair the claim limiter's
  joined_via_share dependency in the same file. dispatching concurrently would
  conflict in exactly the logic neither is allowed to get wrong. OT-123 is
  in-progress now; this unblocks when it merges.
---
