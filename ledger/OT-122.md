...
## Queued 2026-08-19 — held by the 3-builder cap

Not dispatched. OT-115, OT-120 and OT-121 are running, which is the handbook's
limit of three parallel builders. No dependency blocks this and no edit is
needed — dispatch to `builder-deep` as written when a slot frees.

Note the cap is now genuinely enforced again: OT-117 merged and the pairing logic
is live and verified, so this is a real limit rather than the phantom denials
that wedged earlier sessions.
