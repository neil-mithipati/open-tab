...
blocked_reason: >-
  review found TWO HIGH findings and ruled do-not-merge. (1) deleting an account
  removes charge rows from receipts OTHER users own — the builder's claim that
  every charges writer sets from_user_id to the receipt owner is false.
  (2) the modal copy makes an affirmative false claim that only the name is
  retained, when the venmo handle is NOT NULL and definitively survives.
  attempt 2 is specified in the body but was NOT dispatched: the lane hit its
  spend cap ($19.01 of $20.00). owner action: raise the cap or resume, then
  dispatch attempt 2 at builder-deep.
---
