...
blocked_reason: >-
  lane hit its spend cap ($18.43 of $20.00) before a reviewer could be
  dispatched. the builder's work is committed on task/OT-117 with gates green,
  but it is UNREVIEWED and must not be merged on its report alone — it governs
  every future dispatch and its builder explicitly disputes two of this task's
  own acceptance criteria. owner action: raise the cap or resume next session,
  then dispatch the reviewer. separately, `git checkout -- .claude/settings.json
  .claude/hooks/parallel-cap.sh` must be run in the main checkout by the owner or
  the merge will be shadowed by uncommitted regressions.
---
