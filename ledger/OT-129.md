...
blocked_reason: >-
  held on the OT-131 owner decision. the spend cap that originally held this is
  lifted, but the kit re-install replaced bin/finish-worktree with a 70-line
  version that has none of the OT-122 or OT-126 hardening — so section A is
  written against a base that no longer exists in the working tree. sections B
  and C are unaffected and dispatchable now if wanted; split them out rather
  than waiting on A. still the lowest priority of the open tasks: nothing here
  loses data.
---
