...
blocked_reason: >-
  all dispatch paused at the owner's request. the owner stopped every running
  agent (including two builders) to install a new reviewer spec. not re-dispatched
  because doing so would countermand that. ready to dispatch to builder-deep as written; the owner per-account ruling is already recorded in the body and needs no edit.
  resume: owner confirms the spec is installed.
---
