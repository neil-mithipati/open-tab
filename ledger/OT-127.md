...
blocked_reason: >-
  held on the OT-131 owner decision, not on the work. the builder-deep finished
  and committed two commits on task/OT-127, but its reviewer died with the
  session so it is built and unreviewed. more importantly the kit re-install
  replaced all three of this task's files in the main checkout, so the base this
  branch was built on may not survive. do not review or merge until OT-131 is
  answered — under one of the three options this branch is discarded entirely.
---
