...
blocked_reason: >-
  depends on OT-121 merging first. both edit .claude/hooks/parallel-cap.sh, and
  OT-121 is in review right now with an authored commit. this task also builds
  directly on what OT-121 established — that events.jsonl has no per-agent
  activity signal and `session` is the shared parent id — so it should read that
  merged work rather than a description of it.
---
