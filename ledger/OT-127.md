...
blocked_reason: >-
  held at the lane spend cap ($80.50 of $100.00), not by any dependency and not
  by the builder cap. highest-value remaining infrastructure item. fix 1 (deny() calls jq -n, so a
  present-but-broken jq silently allows every dispatch) defeats all five
  fail-opens already closed in that file and is cheap. do this one first.
  resume: owner raises the cap or starts a new session.
---
